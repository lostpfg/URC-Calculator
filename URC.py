...
def Input_Parser_Array_PCN ( file ):
    array = Input_Parser_List ( file ) # Get Input Parsed into a List of Numbers.
    vars_in = int(array[0][0]) # Numbers of Total Variables in Function.
    cubes_in = int(array[1][0]) # Numbers of Total Cubes in Function.
    filenames = [  [ '11' for y in range(0, vars_in)]  for x in range(0, cubes_in) ]
    for x in range (2,len(array)):
        for y in range (1,len(array[x]) ):
            ...
            if number < 0:
                number = -number
                ins = '10'
            filenames[x-2][number-1]=ins
    return filenames,cubes_in,vars_in

# Parses input File in List seperated by lines.
def Input_Parser_List ( file ):
    return [map(int,x.split()) for x in file if x.strip()]
...
def main():
    ...
    sys.stdout.write('-------------------------------------\n')
    filename = raw_input(' #UPR : Enter a file name: ')
    ...
    for x in range(len( Input_F )): print Input_F[x]
